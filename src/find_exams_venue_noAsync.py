...
class FindExamsVenue:
    ...
    This function takes the links of all exams
    ...
    def fetch_and_process(self, link: str, retries: int = 1):
        for i in range(retries):
            try:
                ...
                if table:
                    course_level = (
                        table.find("td", string="Course Level")
                        .find_next_sibling("td")
                        .text
                    )

                    exams_status = (
                        table.find("td", string="Exams Status")
                        .find_next_sibling("td")
                        .text
                    )

                    exam_date = (
                        table.find("td", string="Exam Date")
                        .find_next_sibling("td")
                        .text
                    )

                    exam_time = (
                        table.find("td", string="Exams Time")
                        .find_next_sibling("td")
                        .text
                    )

                    campus = (
                        table.find("td", string="Campus").find_next_sibling("td").text
                    )
                    ...
                    for venue in venues:
                        ...
                        if len(venue_id_range) == 2 and self.binary_search(
                            list(
                                range(
                                    int(venue_id_range[0]), int(venue_id_range[1]) + 1
                                )
                            )
                        ):
                            course_details = {
