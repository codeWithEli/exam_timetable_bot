...
try:
    ...
    cred = credentials.Certificate(AppConfig.FIREBASE_CREDENTIALS_PATH)

    # Initialize Firebase app
    firebase_admin.initialize_app(
        cred, {"storageBucket": AppConfig.FIREBASE_STORAGE_BUCKET}
    )
...
def get_course_code(user_id: str, course) -> str:
    try:
        ...
        if doc.exists:
            course_code = doc.get(f"{sanitized_course}.course_code")
            return course_code
        else:
            return None

    except Exception as e:
        logger.exception(f"🔥Error retrieving course code for user ID {user_id}: {e}")
        return None
...
def set_course_code(user_id: str, course, course_code: str) -> None:
    try:
        ...
        logger.exception(f"🔥Error setting course code for user ID {user_id}: {e}")
...
def save_exams_details(user_id: str, course: str, course_info: dict) -> None:
    ...
    try:
        ...
        if doc.exists:
            sanitized_course = re.sub(r"\W+", "_", course)
            doc_ref.update({sanitized_course: course_info})
        else:
            sanitized_course = re.sub(r"\W+", "_", course)
            doc_ref.create(document_data={sanitized_course: course_info})

    except Exception as e:
        logger.exception(f"🔥Error saving exams details for {user_id}: {e}")
        return None
...
def get_exams_venue(user_id: str) -> str:
    ...
    try:
        ...
        logger.exception(f"🔥Error retrieving exams venue for user ID {user_id}: {e}")
        return None
...
def get_exact_venue(user_id: str, course):
    ...
    try:
        ...
        if doc.exists:
            exact_exams_venue = doc.get(f"{sanitized_course}.Exact_Exams_Venue")
