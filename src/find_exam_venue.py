...
from utils import retry_with_exponential_backoff, logger
...
class FindExamsVenue:
    ...
    async def fetch_and_process(self, session, link: str):
        async with session.get(link) as response:
            ...
            if table:
                ...
                exam_date = (
                    table.find("td", string="Exam Date").find_next_sibling("td").text
                )

                exam_time = (
                    table.find("td", string="Exams Time").find_next_sibling("td").text
                )
                ...
                for venue in venues:
                    ...
                    if len(venue_id_range) == 2 and self.binary_search(
                        list(
                            range(
                                int(venue_id_range[0]),
                                int(venue_id_range[1]) + 1,
                            )
                        )
                    ):
                        course_details = {
                        ...
                        save_exams_details(self.user_id, course_name, course_details)
                        ...
                        save_exams_details(self.user_id, course_name, course_details)
                        set_exact_venue_bool(self.user_id, status=False)
        ...
        async with aiohttp.ClientSession() as session:
            async for link in self.process_links():
                retry_with_exponential_backoff(
                    task=self.fetch_and_process(session, link), max_retries=4, delay=0.2
                )
